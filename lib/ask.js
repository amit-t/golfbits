...
async function askInteractive(opts = {}) {
  ...
  return new Promise(resolve => {
    ...
    const meta = { provider: resolved.provider, command: spec.command, args, prompt };
    let child;
    try {
      child = spawn(spec.command, args, { cwd: opts.cwd || ROOT, stdio });
...
module.exports = { freshQuestions, readQuestions, writeQuestions, buildAskPrompt, buildAskContext, parseSuggestedBitTopic, ask, askInteractive };
