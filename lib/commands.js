...
const { ask, askInteractive } = require("./ask");
...
async function askCommand(argv = []) {
  ...
  if (!question) {
    // No question on the CLI → open the agent interactively, pre-seeded with golf context.
    const c = resolveConfig({ flagAgent: agent });
    console.log(`Opening ${c.provider} with your golf context — ask away in the agent window.`);
    console.log(`(Interactive chats aren't saved. To record a one-shot Q&A: golf.ask "your question")`);
    const result = await askInteractive({ flagAgent: agent });
...
function printUsage() {
  ...
  console.log(`  golfbits ask ["question"] [--agent-flag]        with a question: record a one-shot Q&A; no question: open the agent interactively with golf context`);
