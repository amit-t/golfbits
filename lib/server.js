...
const { APP_DIR, PLAYBOOK_FILE, PLAN_FILE } = require("./paths");
...
function createServer() {
  return http.createServer((req, res) => {
    ...
    if (p === "/api/plan" && req.method === "GET") {
      try {
        return json(res, 200, JSON.parse(fs.readFileSync(PLAN_FILE, "utf8")));
      } catch (e) { return json(res, 404, { error: "content/plan.json not found" }); }
    }
